...
def build_gru_model(
    ...
    Args:
        ...
        hidden_units: Requested channel width. The implementation caps the
            effective width at 48 to keep the model comfortably under the
            100k-parameter budget while preserving deployment safety.
    ...
    # Keep the width in the MCU-friendly range. The effective width is capped
    # to avoid parameter growth while still letting the caller request 32/48/64.
    filters = int(max(32, min(hidden_units, 48)))
